...
async def score_leads(state: DailyPipelineState) -> DailyPipelineState:
    ...
    for i in range(0, len(leads_to_score), batch_size):
        ...
        for lead in batch:
            ...
            recent_notes_text = "\n".join(
                f"  - [{n.get('note_date', '?')[:10]}] {n.get('note_text', '')[:200]}"
                for n in lead_notes[:5]
            ) or "  (no notes)"

            recent_activities_text = "\n".join(
                f"  - [{a.get('activity_date', '?')[:10]}] {a.get('activity_type', '?')}: {a.get('details', '')[:150]}"
...
async def detect_anomalies(state: DailyPipelineState) -> DailyPipelineState:
    ...
    try:
        ...
        this_week_resp = (
            db.table("lead_activities")
            .select("performed_by, id")
            .gte("activity_date", this_week_start)
            .execute()
        )
        ...
        last_week_resp = (
            db.table("lead_activities")
            .select("performed_by, id")
            .gte("activity_date", last_week_start)
            .lt("activity_date", last_week_end)
            .execute()
        )
        ...
        for a in this_week_data:
            uid = a.get("performed_by") or a.get("user_id") or "unknown"
