...
export default function SettingsPage() {
  ...
  function startEditLLM(provider: string) {
  ...
  async function saveLLMProvider() {
    ...
    try {
      ...
      if (res.data) setLLMStatus(res.data)
  ...
  async function saveModelSelection() {
    ...
    setModelError(null)
    try {
      const res = await updateLLMConfig({ model_primary: modelPrimary || null, model_fast: modelFast || null, model_fallback: modelFallback || null })
      if (res.data) {
          ...
          {!llmLoaded ? (
            ...
            <div className="space-y-4">
              {LLM_PROVIDERS.map((provider) => (
                <div key={provider.id} className="flex flex-wrap items-center gap-2 rounded-lg border border-zinc-200/60 bg-zinc-50/50 p-3 dark:border-zinc-700/50 dark:bg-zinc-800/30">
                  ...
                  {editingProvider === provider.id ? (
                    <>
                      <input type={llmShowToken ? 'text' : 'password'} placeholder={`Paste ${provider.label} API key`} value={llmTokenValue} onChange={(e) => { setLLMTokenValue(e.target.value); setLLMError(null) }} className="min-w-[220px] rounded-lg border border-zinc-200 bg-white px-3 py-1.5 text-sm dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-100" />
                      <button type="button" onClick={() => setLLMShowToken((s) => !s)} className="flex items-center gap-1 rounded border border-zinc-200 bg-white px-2 py-1.5 text-xs dark:border-zinc-600 dark:bg-zinc-800">
                        {llmShowToken ? <EyeOff className="h-3.5 w-3.5" /> : <Eye className="h-3.5 w-3.5" />}
                        {llmShowToken ? 'Hide' : 'Show'}
                      </button>
                      <button type="button" onClick={saveLLMProvider} disabled={llmSaving} className="rounded bg-violet-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-violet-700 disabled:opacity-50">
                        {llmSaving ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : 'Save'}
                      </button>
                      <button type="button" onClick={() => { setEditingProvider(null); setLLMError(null) }} className="text-xs text-zinc-500 hover:underline dark:text-zinc-400">Cancel</button>
                    </>
                  ) : (llmStatus as Record<string, unknown>)?.[provider.id] ? (
                    <>
                      ...
                      <button type="button" onClick={() => startEditLLM(provider.id)} className="text-xs font-medium text-violet-600 hover:underline dark:text-violet-400">Change</button>
                    </>
                  ) : (
                    <button type="button" onClick={() => startEditLLM(provider.id)} className="rounded border border-violet-500/50 bg-violet-500/10 px-3 py-1.5 text-xs font-medium text-violet-700 hover:bg-violet-500/20 dark:text-violet-400">Add API key</button>
                  )}
                </div>
              ))}
              {llmError && <p className="text-xs text-red-600 dark:text-red-400" role="alert">{llmError}</p>}
              <div className="mt-4 border-t border-zinc-200/60 pt-4 dark:border-zinc-700/50">
                ...
                <p className="mb-3 text-xs text-zinc-500 dark:text-zinc-400">Primary for complex tasks; fast for scoring/ranking; fallback used if primary fails.</p>
                <div className="grid gap-3 sm:grid-cols-1 md:grid-cols-3">
                  <div>
                    <label className="mb-1 block text-xs font-medium text-zinc-600 dark:text-zinc-400">Primary</label>
                    <select value={modelPrimary} onChange={(e) => { setModelPrimary(e.target.value); setModelError(null) }} className="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-100">
                      {(llmStatus?.models ?? []).map((m) => <option key={m.id} value={m.id}>{m.label}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="mb-1 block text-xs font-medium text-zinc-600 dark:text-zinc-400">Fast</label>
                    <select value={modelFast} onChange={(e) => { setModelFast(e.target.value); setModelError(null) }} className="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-100">
                      {(llmStatus?.models ?? []).map((m) => <option key={m.id} value={m.id}>{m.label}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="mb-1 block text-xs font-medium text-zinc-600 dark:text-zinc-400">Fallback</label>
                    <select value={modelFallback} onChange={(e) => { setModelFallback(e.target.value); setModelError(null) }} className="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-100">
                      {(llmStatus?.models ?? []).map((m) => <option key={m.id} value={m.id}>{m.label}</option>)}
                    </select>
                  </div>
                </div>
                <div className="mt-3 flex items-center gap-2">
                  <button type="button" onClick={saveModelSelection} disabled={modelSaving} className="rounded-lg bg-violet-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-violet-700 disabled:opacity-50">
                    {modelSaving ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Save model selection'}
                  </button>
                </div>
                {modelError && <p className="mt-2 text-xs text-red-600 dark:text-red-400" role="alert">{modelError}</p>}
              </div>
            </div>
          )}
        </div>
