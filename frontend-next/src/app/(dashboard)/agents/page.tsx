...
export default function AgentsPage() {
  ...
  const [sortBy, setSortBy] = useState<'leads' | 'conversion' | 'stale' | 'revenue'>('leads')
  ...
  const fetchProfiles = useCallback(() => {
    ...
    getAgentProfiles()
      ...
      .catch(err => {
        const msg = err.response?.data?.detail ?? err.message ?? 'Failed to load profiles'
  ...
  useEffect(() => {
    ...
    fetchProfiles()
  }, [authLoading, session, fetchProfiles])
  ...
  const sorted = [...profiles].sort((a, b) => {
    ...
    if (sortBy === 'revenue') return (pb.total_revenue || 0) - (pa.total_revenue || 0)
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center py-32 text-center">
        ...
        <h2 className="mb-2 text-xl font-bold text-zinc-700 dark:text-zinc-300">Could not load agent profiles</h2>
        <p className="mb-4 max-w-md text-sm text-zinc-500">{error}</p>
        <button
  ...
  const totalRevenue = profiles.reduce((s, p) => s + (p.performance?.total_revenue || 0), 0)
  const totalPitched = profiles.reduce((s, p) => s + (p.performance?.total_price_pitched || 0), 0)
  ...
  const topRevAgent = [...profiles].sort((a, b) => (b.performance?.total_revenue || 0) - (a.performance?.total_revenue || 0))[0]
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-8">
        ...
        <KpiCard icon={IndianRupee} label="Total Revenue" value={totalRevenue > 10000000 ? `₹${(totalRevenue / 10000000).toFixed(1)}Cr` : `₹${(totalRevenue / 100000).toFixed(0)}L`} color="text-green-600" small />
        <KpiCard icon={DollarSign} label="Price Pitched" value={totalPitched > 10000000 ? `₹${(totalPitched / 10000000).toFixed(1)}Cr` : `₹${(totalPitched / 100000).toFixed(0)}L`} color="text-purple-600" small />
        <KpiCard icon={Award} label="Avg Deal" value={`₹${(avgDealSize / 100000).toFixed(1)}L`} color="text-cyan-500" small />
        <KpiCard icon={Award} label="Top Revenue" value={topRevAgent?.name || '-'} color="text-indigo-500" small />
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        {[
          ...
          { key: 'revenue' as const, label: 'Top Revenue' },
            ...
            <div key={profile.id} className="rounded-xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
              ...
              >
                ...
                <div className="flex items-center gap-6">
                  ...
                  <Stat label="Revenue" value={p.total_revenue ? `₹${(p.total_revenue / 100000).toFixed(0)}L` : '-'} className="text-green-600" />
                  {(p.stale_30 || 0) > 0 && (
                ...
                <div className="border-t border-zinc-100 px-5 py-4 dark:border-zinc-800">
                  <div className="grid gap-4 lg:grid-cols-3">
                    ...
                    <div className="space-y-3">
                      ...
                      <h4 className="text-xs font-semibold text-zinc-500">Revenue</h4>
                      <div className="grid grid-cols-2 gap-2">
                        <div className="rounded-lg bg-green-50 px-2.5 py-1.5 dark:bg-green-500/10">
                          <p className="text-xs font-semibold text-green-700 dark:text-green-400">{p.total_revenue ? `₹${(p.total_revenue / 100000).toFixed(1)}L` : '₹0'}</p>
                          <p className="text-[10px] text-green-600">Revenue</p>
                        </div>
                        <div className="rounded-lg bg-purple-50 px-2.5 py-1.5 dark:bg-purple-500/10">
                          <p className="text-xs font-semibold text-purple-700 dark:text-purple-400">{p.total_price_pitched ? `₹${(p.total_price_pitched / 100000).toFixed(1)}L` : '₹0'}</p>
                          <p className="text-[10px] text-purple-600">Pitched</p>
                        </div>
                        <MiniStat label="Avg Deal" value={p.sales_done && p.total_revenue ? `₹${(p.total_revenue / p.sales_done / 100000).toFixed(1)}L` : '-'} />
                        <MiniStat label="Leads" value={p.total_leads || 0} />
                      </div>
