...
**Location:** `Onsite/sales-intelligence`  
...
```bash
cd Onsite/sales-intelligence/backend
python3 -m venv venv
...
```bash
./Onsite/scripts/start-backend.sh
```
...
```bash
cd Onsite/sales-intelligence/frontend-next
...
- Or run `python3 scripts/setup_database.py` from `Onsite/` (uses `backend/.env` for Supabase URL + service key).
