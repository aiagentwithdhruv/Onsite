...
async def get_today_brief(user: dict = Depends(get_current_user)):
    ...
    The brief is generated by a scheduled job (morning_brief_agent) that runs
    each day. This endpoint retrieves the pre-generated brief.
    """
    try:
...
async def get_rep_brief_today(rep_id: str, user: dict = Depends(require_manager)):
    ...
    Managers can view any rep's brief to understand their day and priorities.
    """
    try:
        db = get_supabase_admin()
        today = _today_str()
        ...
        rep_result = db.table("users").select("id, full_name, role").eq("id", rep_id).single().execute()
        ...
        if not result.data:
            return {
                ...
                "rep": {
                    "id": rep_info["id"],
                    "name": rep_info.get("full_name"),
                    "role": rep_info.get("role"),
                },
                "message": f"No brief generated for {rep_info.get('full_name', 'this rep')} today.",
                "brief_date": today,
            }
