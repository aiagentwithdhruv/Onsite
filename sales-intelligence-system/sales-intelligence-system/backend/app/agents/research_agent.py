...
async def analyze_notes(state: ResearchState) -> ResearchState:
    ...
    for note in crm_notes:
        notes_text += (
            ...
            f"Type: {note.get('note_type', 'general')}\n"
...
async def match_past_wins(state: ResearchState) -> ResearchState:
    ...
    try:
        ...
        if similar_deals:
            ...
            deal_notes_resp = (
                db.table("notes")
                .select("lead_id, content, note_type")
                .in_("lead_id", top_deal_ids)
                .in_("note_type", ["closing_note", "call_summary", "general"])
                .order("created_at", desc=True)
                .limit(10)
                .execute()
            )
            ...
            for n in deal_notes:
                notes_by_deal.setdefault(n["lead_id"], []).append(n.get("content", ""))
